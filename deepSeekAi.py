...
def r1test(code):
    ...
    data_sample = df.tail(10)
    data_str = data_sample.to_markdown()
    data_summary = f"数据时间范围：{df['DateTime'].min()} 至 {df['DateTime'].max()}"

    user_prompt = f"""
        你是一位专业且严谨的金融量化分析师，需要基于提供的s
        股票日线数据进行多维度技术分析(如缠论、均线、MACD、RSI、量价关系等)。请严格按照以下流程处理，最终输出指定格式结果：
    ...
    if result['is_complete']:
        ...
        if result['usage']['total_tokens'] > 8000:
