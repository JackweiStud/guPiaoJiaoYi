...
def build_prompt_from_view(view: Dict) -> str:
    ...
    输入参数:
        view (Dict): 包含市场数据的字典，包括指数、涨跌比、行业板块、全球信号等信息
        
    返回值:
        str: 格式化的AI分析提示词，用于生成市场分析结论
    """
    ...
    bottom_names = [s.get("etf_name", "") for s in sectors[-3:]] if len(sectors) >= 3 else [s.get("etf_name", "") for s in sectors[-len(sectors):]]
...
def _deepseek_chat(prompt: str) -> str:
    ...
    try:
        resp = requests.post(url, headers=headers, json=payload, timeout=60*3)
