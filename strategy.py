...
def simple_ma_strategy(data, short_window, long_window, plot_chart=False):
    """
    简单均线交叉策略
    ...
    MaRateUp = 1.1
    ma_buy_condition = ((ma_state_diff == 1) & (data['Volume'] > (volume_mavg * MaRateUp)))
    # RSI超卖
    rsiValueThd = 25
    rsi_buy_condition = (rsi < rsiValueThd) & (data['Volume'] > volume_mavg ) 
    ...
    divergence_df.to_csv('divergence_ratio.csv', index=False)
    ...
    # 新增功能：将信号保存到CSV文件
    signals['signal'].to_csv('temp_strategy.csv', header=True)
    
    # 新增绘图功能
    if plot_chart:
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 9), sharex=True, gridspec_kw={'height_ratios': [3, 1]})
        fig.suptitle('K线图, 均线与交易信号', fontsize=16)
        ...
        ax2.plot(plot_data_reset.index, plot_data_reset['volume_ma'] / 10000, color='blue', linestyle='--', linewidth=1, label='7日均量')
        ...
        fig.tight_layout(rect=[0, 0, 1, 0.96]) # 为主标题调整布局
        plt.show()
...
def run_backtest(data,
    ...
    portfolio_info.to_csv('SellAndBuy.csv', float_format='%.2f')
    return portfolio


def calculate_performance(portfolio_df, initial_capital):
    """
    计算并打印绩效指标
...
def plot_performance(portfolio_df, benchmark_data=None):
    ...
    # 计算5日、10日、21日均线
    plot_data['ma5'] = plot_data['CloseValue'].rolling(window=5, min_periods=1).mean()
    plot_data['ma21'] = plot_data['CloseValue'].rolling(window=21, min_periods=1).mean()
    # 计算5日均量线
    plot_data['volume_ma'] = plot_data['Volume'].rolling(window=10, min_periods=1).mean()

    # === 窗口A: K线图和成交量 ===
    fig1, (ax0, ax3) = plt.subplots(2, 1, figsize=(18, 12), 
                                     sharex=True,
                                     gridspec_kw={'height_ratios': [3, 1]})
    fig1.suptitle('K线图与成交量', fontsize=18)
    ...
    # 绘制均线
    ax0.plot(plot_data.index, plot_data['ma5'], color='blue', linestyle='-', linewidth=1.5, label='5日均线')
    ax0.plot(plot_data.index, plot_data['ma21'], color='purple', linestyle='-', linewidth=1.5, label='21日均线')
    ...
    # 绘制5日均量线
    ax3.plot(plot_data.index, plot_data['volume_ma'] / 10000, color='blue', linestyle='--', linewidth=1, label='10日均量')
...
def testFunc(filepath, 
        ...
        # 2. 生成策略信号
        signals = simple_ma_strategy(etf_data, short_window=short_window, long_window=long_window, plot_chart=False)
        ...
        performance_stats = calculate_performance(portfolio_results_df, initial_capital)
...
if __name__ == "__main__":
   ...
   #filepath="D:\\Code\\Ai\\jinrongTest\\github\\stock_data\\588180_Day.csv"
   short_window =5
   long_window =21
   buy_increment = 1 #1 0.5
   sell_decrement = 1
   testFunc(filepath,short_window,long_window,buy_increment_pct_of_initial_capital=buy_increment,   
